...
describe('CHANGE LANGUAGE', () => {
  ...
  it('Should change Language', () => {
    ...
    saga
      .next(setLanguage)
      .next().put({ type: actionTypes.CHANGE_LANGUAGE, payload: i18n.lang })
      .next().isDone();
  });
});
...
describe('Notification', () => {
  ...
  it('Should show a notification', () => {
    ...
    saga
      .next().put({ type: actionTypes.NOTIFICATION_CALL, payload: true, title: action.title, style: action.style })
      .next(delay())
      .next().put({ type: actionTypes.NOTIFICATION_CALL, payload: false, title: action.title })
      .next().isDone();
  });
});
...
describe('File Uploaded', () => {
  ...
  it('Save the json file on server', () => {
    ...
    saga
      .next().put({ type: actionTypes.JSON_FILE_UPLOADED_SUCCESSFUL, payload: true })
      .next().isDone();
  });
});
